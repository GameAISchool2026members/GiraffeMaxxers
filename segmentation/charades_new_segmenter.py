...
<head>
  ...
  <style>
    ...
    * {
    ...
    #emotionBar {
      background: linear-gradient(90deg, #c792ff, #79b8ff);
    }
...
airplane</textarea>
      ...
      <div class="control">
        <label for="emotionTarget">Target Emotion (face-api)</label>
        <select id="emotionTarget">
        ...
        <div class="hint">Scored independently from CLIP, off the largest face in frame.</div>
      </div>
    ...
    <section class="stage">
      <div class="views">
        ...
        <div class="card">
          <h2>Emotion (face-api)</h2>
          <div class="meter">
            <div class="meter-row"><span>Emotion score</span><strong id="emotionScore">--</strong></div>
            <div class="bar"><div id="emotionBar"></div></div>
          </div>
          <div class="top-labels" id="emotionTop">Load CLIP and start scoring to read emotion.</div>
        </div>
  ...
  <script type="module">
    ...
    const elements = {
      ...
      emotionTarget: byId("emotionTarget"),
      emotionScore: byId("emotionScore"),
      emotionBar: byId("emotionBar"),
      emotionTop: byId("emotionTop"),
    ...
    async function requestServerSegmentation() {
      const canvas = document.createElement("canvas");
      canvas.width = elements.video.videoWidth || 640;
      canvas.height = elements.video.videoHeight || 480;

      const ctx = canvas.getContext("2d");
      drawMirroredCover(ctx, elements.video, canvas.width, canvas.height);
    ...
    function startSegmentationLoop() {
      ...
      state.segmentationTimer = setInterval(async () => {
        if (!state.webcamReady || state.segmentationBusy || state.scoringNow) return;
        state.segmentationBusy = true;
        try {
          await requestServerSegmentation();
        } catch (error) {
          console.warn(error);
        } finally {
    ...
    async function loadFaceModels() {
      ...
      try {
        ...
        console.warn("face-api model load failed", error);
        elements.emotionTop.textContent = "Emotion model failed to load (see console).";
      }
    }
    ...
    async function scoreEmotion() {
      if (!state.faceReady) {
        elements.emotionScore.textContent = "loading";
        elements.emotionTop.textContent = "Emotion model still loading...";
        return;
      }
      if (!state.webcamReady) return;

      const vid = elements.video;
      ...
      const detections = await faceapi
        .detectAllFaces(snapCanvas, new faceapi.TinyFaceDetectorOptions({ inputSize: 224, scoreThreshold: 0.4 }))
        .withFaceExpressions();

      if (!detections.length) {
        elements.emotionScore.textContent = "no face";
        elements.emotionBar.style.width = "1%";
        elements.emotionTop.textContent = "No face detected.";
        state.lastEmotionScore = null;
        return;
      }

      // Single subject: keep the largest face, matching the CLIP app's one-subject framing.
      ...
      const targetKey = elements.emotionTarget.value;
      const score = expr[targetKey] ?? 0;
      state.lastEmotionScore = score;
      updateEmotionDisplay(expr, targetKey, score);
    }

    function updateEmotionDisplay(expr, targetKey, score) {
      const pct = Math.round(score * 1000) / 10;
      elements.emotionScore.textContent = `${pct.toFixed(1)}%`;
      elements.emotionBar.style.width = `${Math.max(1, pct)}%`;
      elements.emotionTop.innerHTML = EMOTION_KEYS
        .map((key) => ({ key, value: expr[key] ?? 0 }))
        .sort((a, b) => b.value - a.value)
        .slice(0, 5)
        .map(({ key, value }) => {
          const line = `${key}: ${(value * 100).toFixed(1)}%`;
          return key === targetKey ? `<strong>${line}</strong>` : line;
        })
        .join("<br>");
    }
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
      ...
      mask_png = segment_jpeg_to_mask_png(body)
